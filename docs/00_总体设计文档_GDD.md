...
> 版本：v0.3 | MVP阶段：县令三年 / 知府独立开局  
> 核心理念：AI Agent驱动的明朝官场博弈模拟  

> 当前统一口径：**游戏回合单位为“月”**。文档中保留的“季度/春夏秋冬”表述，仅用于描述季节性事件分组，不再表示实际操作回合。
...
```
- 玩家主线职位：`知县（首任） → 县/州地方历练（后续任期可为知县或知州） → 知府 → 巡抚（可兼中央一级部门副职） → 内阁成员 → 内阁首辅`
- 玩家首任固定为知县；后续地方任职可能为知县或知州，知州不是升任知府的硬性前置职位
- 产品入口层面允许直接新开知府游戏，以独立体验知府阶段玩法
- 非玩家晋升轨道（关键NPC）：布政使、按察使、六部、都察院言官

### 2.2 回合制节奏

**当前统一方案**：每月一个回合，每年12回合，三年共36回合。  

| 季节分组 | 月份 | 主题 | 核心行动 |
|---------|------|------|----------|
| 春 | 正月~三月 | 规划与播种 | 制定年度投资方向、分配预算、安排春耕 |
| 夏 | 四月~六月 | 执行与应变 | 处理突发事件（灾害、案件、流民）、执行政策 |
| 秋 | 七月~九月 | 收获与征税 | 秋收结算、执行税收、处理抗税/欠税 |
| 冬 | 十月~腊月 | 社交与考核 | 官场社交、中央政策表态、年终考核 |



### 2.3 行动力系统
- 每回合 **3个行动点**（AP），全年共36个AP
- 不同操作消耗不同AP，具体待定
...
│   ├── 刑部（尚书1 + 侍郎3）← 司法
│   └── 工部（尚书1 + 侍郎3）← 工程水利
├── 都察院（言官机构, 左都御史*1 二品, 左副都御史*1 五品，监察御史*6 七品）
└── 地方(地方行政结果：省(巡抚)=》府(知府)=》州/县(知州/知县)
    ├── 总督（一品，多省一个,但不是固定，一般是需要而设置，类似陕甘总督，湖广总督, 游戏初期先省略）
    ├── 巡抚（二品，每个省一个）
    ├── 省下辖司（五品：布政司/按察司, 每个省每个职位各1个）
    ├── 知府（三/四品， 每个府一个）
    ├── 知州（六品，州）── 与县平级，更繁荣
    └── 知县（七品，县）← 玩家起点
```
...
### 4.1 初始设定
- 身份：新科进士，初授七品县令
- 测试阶段额外露出 **县域类型选择**（财赋核心/宗族治理/沿海治理/黄淮灾荒），用于定向验证不同类型的数值演进与平衡性
...
- **声望维度**（外部评价，半隐藏——玩家可看到5档粗略分级，但分级与真实数值有偏差）：
  ...
  - 威名：必要时强硬的口碑；默认为40，根据对抗地主/强硬执法等行为调整；威名 ≥ 60 时提升谈判胜率，但也招致强势地主打报告上级（见 01_人物模型.md §9）
...
| 层级 | 属性 | 说明 |
|------|------|------|
| 核心属性 | 智力、魅力、忠诚 | 基础能力，影响决策质量、社交效能和抗拉拢能力 |
| 性格属性（隐藏） | 孤僻—合群（sociability）、感性—理性（rationality）、温顺—强硬（assertiveness） | 影响行为风格和谈判软硬度；低值为左端，高值为右端 |
| 政治理念（隐藏） | 社稷—黎民、集权—分权、现实—理想 | 影响政策倾向和派系归属；玩家理念从 0.5 出发随行为漂移 |
| 名誉属性（半隐藏） | 清名、能名、人缘、威名 | 5档粗略分级对外可见，有 ±10 随机偏差 |
...
| 02b | 府域经营模型 | 知州/知府阶段的委托治理、下辖县管理、府域经济 |
...
| 08 | 官场体系 | 行政层级、君主原型、派系系统、官场Agent |
