...
(function () {
  ...
  var INVEST_DEFS = [
    { action: "reclaim_land", name: "开垦荒地", cost: 50, desc: "为目标村庄增加200亩耕地，下个秋季完成", needsVillage: true },
  ...
  function renderVillages() {
    ...
    g.county_data.villages.forEach(function (v) {
      var tr = h("tr", "",
        ...
        "<td>" + Math.round(v.morale) + "</td>" +
        "<td>" + Math.round(v.security) + "</td>" +
  ...
  function renderReport(report) {
    ...
    if (report.events && report.events.length > 0) {
      ...
      report.events.forEach(function (evt) {
        var item = h("div", "report-event", evt);
